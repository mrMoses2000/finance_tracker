...
Цель: единое хранилище данных (Postgres на сервере) и единый API (наш backend на :4000), к которому могут безопасно обращаться и веб-клиент, и bot‑прослойка (Gemini CLI), и будущие мессенджеры. Решение: без n8n, своя тонкая прослойка.
...
## 1.1) Bot Gateway (тонкая прослойка вместо clawd.bot)
- Отдельный небольшой сервис на Stockholm‑сервере.
- Принимает сообщения (Telegram webhooks), определяет пользователя.
- Читает память/контекст из нашей БД (Frankfurt).
- Вызывает `gemini` CLI как дочерний процесс.
- Отправляет строгое JSON‑действие в наш API `/api/bot/actions`.
- Хранит логи и упрощает отладку.

## 2) Bot-API и схема действий
Добавить отдельный, строгий endpoint:
