...
12) Можно передавать env‑переопределения как `VAR=value ./run.sh` или `./run.sh VAR=value`.
13) Авто‑режим доступен в меню скрипта (без длинных параметров).
14) `.env` можно оставлять с пробелами в значениях — скрипт читает безопасно.
